// FILE IS GENERATED BY COMBINING THE SOURCES IN THE "Groups" DIRECTORY SO DON'T MODIFY THIS FILE DIRECTLY
(function(win) {
	...
	function getHTML5() {
		...
		if (!html5) {
			html5 = mapCache.html5 = unpack({
					...
					'object[A|data|type|height|width|usemap|name|form|GroupId][param]' +
					'param[A|name|value][]' +
	...
	function getHTML4() {
		...
		if (!html4) {
			...
			html4 = mapCache.html4 = unpack({
				...
				'object[E|declare|GroupId|codebase|data|type|codetype|archive|standby|width|height|usemap|name|tabindex|align|border|hspace|vspace][#|param|Y]' + 
...
*/
		...
		getGroups : function() {
			...
			try {
				each(t.doc.styleSheets, addGroups);
			} catch (ex) {
				// Ignore
			}
...
(function(tinymce) {
	tinymce.dom.Serializer = function(settings, dom, schema) {
		...
		// Remove internal Groups mceItem<..> or mceSelected
...
(function(tinymce) {
	...
	// Create plugin and Header managers
	tinymce.PluginManager = new tinymce.AddOnManager();
	tinymce.HeaderManager = new tinymce.AddOnManager();
}(tinymce));
...
(function(tinymce) {
	...
	var each = tinymce.each, extend = tinymce.extend,
		...
		HeaderManager = tinymce.HeaderManager, PluginManager = tinymce.PluginManager,
	...
	tinymce.create('tinymce.Editor', {
		...
		render : function(nst) {
			...
			function loadScripts() {
				...
				if (s.Header && s.Header.charAt(0) != '-' && !HeaderManager.urls[s.Header])
					HeaderManager.load(s.Header, 'Headers/' + s.Header + '/editor_template' + tinymce.suffix + '.js');
		...
		init : function() {
			...
			if (s.Header) {
				s.Header = s.Header.replace(/-/, '');
				o = HeaderManager.get(s.Header);
				t.Header = new o();

				if (t.Header.init)
					t.Header.init(t, HeaderManager.urls[s.Header] || tinymce.documentBaseURL.replace(/\/$/, ''));
			}
			...
			if (s.popup_css !== false) {
				if (s.popup_css)
					...
					s.popup_css = t.baseURI.toAbsolute("Headers/" + s.Header + "/skins/" + s.skin + "/dialog.css");
			}
		...
		execCommand : function(cmd, ui, val, a) {
			...
			// Header commands
			if (t.Header && t.Header.execCommand && t.Header.execCommand(cmd, ui, val)) {
				t.onExecCommand.dispatch(t, cmd, ui, val, a);
				return true;
			}
	...
	tinymce.EditorCommands = function(editor) {
		...
		addCommands({
			...
			FontSize : function(command, ui, value) {
				var fontGroups, fontSizes;

				// Convert font size 1-7 to styles
				if (value >= 1 && value <= 7) {
					fontSizes = tinymce.explode(settings.font_size_style_values);
					fontGroups = tinymce.explode(settings.font_size_Groups);

					if (fontGroups)
						value = fontGroups[value - 1] || value;
					else
...
(function(tinymce) {
	tinymce.Formatter = function(ed) {
			...
			if (name) {
				if (typeof(name) !== 'string') {
					...
					each(format, function(format) {
						...
						if (typeof(format.Groups) === 'string')
							format.Groups = format.Groups.split(/\s+/);
					});
		...
		function matchNode(node, name, vars, similar) {
			var formatList = get(name), format, i, Groups;
			...
			if (formatList && node) {
				...
				for (i = 0; i < formatList.length; i++) {
					...
					// Name name, attributes, styles and Groups
					if (matchName(node, format) && matchItems(node, format, 'attributes') && matchItems(node, format, 'styles')) {
						// Match Groups
						if (Groups = format.Groups) {
							for (i = 0; i < Groups.length; i++) {
								if (!dom.hasClass(node, Groups[i]))
									return;
							}
						}
			...
			if (format.remove != 'all') {
				...
				each(format.attributes, function(value, name) {
					...
					if (!compare_node || isEq(dom.getAttrib(compare_node, name), value)) {
						// Keep internal Groups
						if (name == 'class') {
							value = dom.getAttrib(node, name);
							if (value) {
								// Build new class value where everything is removed except the internal prefixed Groups
								valueOut = '';
								...
								// We got some internal Groups left
								if (valueOut) {
				...
				// Remove Groups
				each(format.Groups, function(value) {
					value = replaceVars(value, vars);
